...
Experiment = function() {
	...
	this.exp5task = function() {
		...
		//	buildings = 10; // 10 buildings on the last task
		$("#packagetxt").hide();
		var res = map.randomMap(buildings);
		//var coords = res[0];
		delivery_game = true;
	};
	...
	var nextTask = function() {
		if (controls.pathlength > 0 && data["exp"+exp_properties.expno]["task"+exp_properties.taskno]) {
			data["exp"+exp_properties.expno]["task"+exp_properties.taskno].pathlength = Math.round(controls.pathlength * DISTSCALE); // in m
			data["exp"+exp_properties.expno]["task"+exp_properties.taskno].pathcoords = coords;
		}

		exp_properties.taskno++;
		if (exp_properties.taskno <= taskNumbersPerExperiment[exp_properties.expno]) {
			// reset path length and coords
	...
	this.onEnter = function() {
		...
		if (mapcanvasshown && !recall_task) {
			...
			if (!mapcanvasclicked) {
				...
				//this.recallcues = [-1, -1].concat(this.recallcues);
				this.recallcueid = 0;
				...
				if (rnames.length == bnames.length) {
					...
					for (var i = 0; i < rnames.length; i++) {
						for (var j = 0; j < bnames.length; j++) {
							if (stringDifference(rnames[i], bnames[j]) <= MAXSTRDIFF) {
								rnames[i] = bnames[j];
								correct++;
							}
						}
...
function showRecallOverlay(id, rno) {
	...
	html += " (and the buildings that go with it), and then press Enter. ";
